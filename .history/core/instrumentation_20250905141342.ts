...
function isOpenTelemetryAlreadyConfigured(): boolean {
  try {
    ...
    // More thorough check for existing providers
    const isConfigured = 
      existingProvider &&
    ...
    // Additional check: see if APIs are already registered
    const apiState = (opentelemetry as any)?._global?.api;
    const hasRegisteredAPIs = apiState && (
      apiState.trace?._proxyTracerProvider || 
      apiState.context?._proxyContextManager || 
      apiState.propagation?._proxyPropagator
    );
    
    if (hasRegisteredAPIs) {
      console.log("OpenTelemetry APIs already registered, using existing setup");
      return true;
    }
...
function initializeInstrumentation() {
  ...
  try {
    ...
    // Safer registration approach - avoid duplicate registration
    try {
      // Only register if we can safely do so
      const currentProvider = trace.getTracerProvider();
      if (currentProvider && currentProvider.constructor.name === "ProxyTracerProvider") {
        provider.register();
        console.log("Continue OpenTelemetry provider registered successfully");
      } else {
        console.log("Cannot safely register OpenTelemetry provider, using existing one");
        // Just set the global tracer provider without calling register
        trace.setGlobalTracerProvider(provider);
        console.log("Continue OpenTelemetry provider set as global (without registration)");
      }
    } catch (registrationError: any) {
      console.warn("Failed to register OpenTelemetry provider:", registrationError.message);
      console.log("Attempting fallback: setting as global tracer provider only");
      try {
        trace.setGlobalTracerProvider(provider);
        console.log("Continue OpenTelemetry provider set as global tracer provider");
      } catch (fallbackError: any) {
        console.error("Failed to set global tracer provider:", fallbackError.message);
        console.log("Will use existing OpenTelemetry setup");
      }
    }
