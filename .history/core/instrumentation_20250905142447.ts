...
function initializeInstrumentation() {
  ...
  try {
    ...
    // Set the global tracer provider directly - avoid calling provider.register()
    // since the APIs are likely already registered and that causes duplicate registration errors
    try {
      trace.setGlobalTracerProvider(provider);
      console.log("Continue OpenTelemetry provider set as global tracer provider");
    } catch (setProviderError: any) {
      console.error("Failed to set global tracer provider:", setProviderError.message);
      console.log("Will continue without custom OpenTelemetry provider");
    }
